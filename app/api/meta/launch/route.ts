  ...
  try {
    ...
    if (!body.headline || !body.description || !body.imageUrl) {
      return NextResponse.json(
        { error: "Missing required fields: headline, description, and imageUrl are required" },
        { status: 400 }
      );
    }
    ...
    const mockResponse = {
      campaignId: `camp_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`,
      adSetId: `adset_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`,
      creativeId: `creative_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`,
