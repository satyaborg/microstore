...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (!aiResponse.suppliers || !Array.isArray(aiResponse.suppliers) || aiResponse.suppliers.length !== 3) {
      ...
      const formattedSuppliers = aiResponse.suppliers.map((supplier: any, index: number) => ({
...
function generateMockSuppliers(product: string): Supplier[] {
  ...
  return templates.map((template, index) => ({
    ...
    shippingDays: `${3 + Math.floor(Math.random() * 8)}-${7 + Math.floor(Math.random() * 12)} days`,
