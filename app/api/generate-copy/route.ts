...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const headline = aiResponse.headline
        ? smartTruncate(aiResponse.headline, 40)
        : "";
      const description = aiResponse.description
        ? smartTruncate(aiResponse.description, 90)
        : "";
      ...
      if (aiResponse.headline && aiResponse.headline.length > 40) {
        console.warn(
          "AI headline truncated:",
          aiResponse.headline.length,
          "chars ->",
          headline.length,
          "chars"
        );
...
function generateMockCopy(product: string): {
  headline: string;
  description: string;
  source: string;
} {
